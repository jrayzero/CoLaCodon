...
void versMsg(llvm::raw_ostream &out) {
  out << "Cola " << SEQ_VERSION_MAJOR << "." << SEQ_VERSION_MINOR << "."
      << SEQ_VERSION_PATCH << "\n";
}

const std::vector<std::string> &supportedExtensions() {
  static const std::vector<std::string> extensions = {".seq", ".py", ".cola"};
  return extensions;
}
...
ProcessResult processSource(const std::vector<const char *> &args) {
  ...
  if (input != "-" && std::find_if(exts.begin(), exts.end(), [&](auto &ext) {
    ...
    seq::compilationError(
        "input file is expected to be a .seq/.py/.cola file, or '-' for stdin");
